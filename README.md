...
> 基于 Spring Boot 4 + MyBatis + Vue 3 的人力资源管理系统  
> 涵盖基础架构管理、员工 OA 流程（打卡考勤、请假审批）及月自动化薪资计算

---

## 目录

1. [MVC 分层架构与数据流](#1-mvc-分层架构与数据流)
2. [系统功能结构](#2-系统功能结构)
3. [项目文件结构](#3-项目文件结构)
4. [表结构与关系模式](#4-表结构与关系模式)
5. [后端框架依赖说明](#5-后端框架依赖说明)
6. [前端工程化分析](#6-前端工程化分析)
7. [模块功能代码对照](#7-模块功能代码对照)
8. [测试用例与权限分组](#8-测试用例与权限分组)
9. [附录](#9-附录)
...
- **`common/Result.java`** — 所有 Controller 返回 `Result` 对象，包含 `code`（200 成功/500 失败）、`msg`、`data` 三字段，序列化为
  JSON 响给前端。
- **`common/GlobalExceptionHandler.java`** — `@RestControllerAdvice`
  全局异常捕获，将未处理的异常转化为 `Result.fail(...)` 返回，避免堆栈泄露。
...
1. 浏览器发起 HTTP 请求到 `localhost:8080`。
2. `CorsFilter` 放行跨域（开发阶段允许所有来源）。
3. `AuthInterceptor` 拦截 `/api/**` 路径：
...
4. `Controller` 根据映射调用相应的 `Service` 方法，并返回 `Result`。
5. `Service` 层执行业务逻辑，如有需要调用一个或多个 `Mapper` 接口。
6. `Mapper` 通过 MyBatis 执行 SQL 操作 MySQL 8.0 数据库。
7. 数据沿原路径返回，由 Jackson 3 序列化为 JSON 响应。
...
**在项目中的角色**：**仅用于实体注解**（`@Entity`、`@Table`、`@Id`、`@ManyToOne`、`@OneToMany`、`@Transient` 等），所有实际数据库查询走
MyBatis。不使用 JPA Repository。
...
项目前端经历了从**单文件 SPA** 到 **Vite 工程化**的改造。旧版 `static/index.html` 保留作为备参，当前活动前端为 `vuehr/`
目录下的独立工程。
...
| 人事部   | hr    | 123456 | 小刚 (EMP003) | 最高管理权限        |
| 普通员工  | ming  | 123456 | 小明 (EMP001) | OA 打卡/请假/查看薪资 |
| 普通员工  | hong  | 123456 | 小红 (EMP002) | OA 打卡/请假/查看薪资 |

### 9.4 薪资算法速查

```
应出勤天数  = 该月周一~周五数量（入职当月从入职日起算）
实际出勤    = 状态为"正常"或"迟到"的天数
请假天数    = 已批准的请假（按在当月的实际重叠天数计算）
缺勤天数    = 应出勤 - 实际出勤 - 请假（不小于 0）
缺勤扣款    = (底薪 / 应出勤) × 缺勤天数
满勤奖      = 缺勤=0 且 请假=0 → +200 元，否则 0
实发工资    = 底薪 + 满勤奖 + 绩效 − 缺勤扣款
```
